...
class PriorityQueue {
  // ... (Our existing, correct PriorityQueue code) ...
...
export function dijkstra(grid, startNode, endNode) {
  ...
  while (!priorityQueue.isEmpty()) {
    ...
    // If we pop a node that we have already visited, skip it.
    // This handles the "lazy update" where we might have multiple versions
    // of the same node in the PQ with different distances.
    if (closestNode.isVisited) continue;
...
function updateUnvisitedNeighbors(node, grid, priorityQueue) {
  const unvisitedNeighbors = getUnvisitedNeighbors(node, grid);
  for (const neighbor of unvisitedNeighbors) {
    // The +1 represents the "weight" of the edge between nodes.
